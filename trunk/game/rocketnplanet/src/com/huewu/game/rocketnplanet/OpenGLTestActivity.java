...
public class OpenGLTestActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		GLSprite[] spriteArray = new GLSprite[robotCount + 1 + 10];    
		...
		for (int x = 0; x < robotCount; x++) {
			...
			renderableList.add(robot);
		}
		
		for (int x = robotCount; x < robotCount + 10; x++) {
			GLSprite meteor;
			// Our robots come in three flavors.  Split them up accordingly.
			...
			// Add this robot to the spriteArray so it gets drawn and to the
			// renderableArray so that it gets moved.
			spriteArray[x + 1] = meteor;
			renderableList.add(meteor);
		}		
		...
		handler.setRenderables(renderableList);
